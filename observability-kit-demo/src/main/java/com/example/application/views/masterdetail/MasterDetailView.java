...
public class MasterDetailView extends Div implements BeforeEnterObserver {
    ...
    public MasterDetailView(SamplePersonService samplePersonService) {
        ...
        grid.setPageSize(550);

        // Configure Grid
        grid.addColumn(new ComponentRenderer<>(
                person -> new TextField(person.getFirstName())))
                .setHeader("First Name").setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(
                person -> new TextField(person.getLastName())))
                .setHeader("Last Name").setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(
                person -> new TextField(person.getEmail()))).setHeader("Email")
                .setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(
                person -> new TextField(person.getPhone()))).setHeader("Phone")
                .setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(person -> {
            DatePicker datePicker = new DatePicker();
            datePicker.setValue(person.getDateOfBirth());
            datePicker.addValueChangeListener(
                    event -> person.setDateOfBirth(event.getValue()));
            return datePicker;
        })).setHeader("Date Of Birth").setAutoWidth(true);
        grid.addColumn(new ComponentRenderer<>(
                person -> new TextArea(person.getOccupation())))
