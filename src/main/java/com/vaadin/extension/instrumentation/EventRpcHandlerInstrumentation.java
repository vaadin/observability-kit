...
public class EventRpcHandlerInstrumentation implements TypeInstrumentation {
    ...
    public static class MethodAdvice {
        ...
        public static void onEnter(
            ...
            span.setAttribute("element-tag", element.getTag());

            if (eventType != null) {
                span.setAttribute("event", eventType);
                if (element.hasProperty("opened")) {
                    span.setAttribute("opened", element.getProperty("opened"));
                    // Note! False means opening as we are checking before the event has happened!
                    if (element.getProperty("opened", false)) {
                        eventType = element.getTag() + " opening";
                    } else {
                        eventType = element.getTag() + " closing";
                    }
                }
                // If possible add active view class name as an attribute to the span
                if (node.getOwner() instanceof StateTree) {
                    span.setAttribute("view",
                ...
                span.updateName(element.getTag() + " :: " + eventType);
